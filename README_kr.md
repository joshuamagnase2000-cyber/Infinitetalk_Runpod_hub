...
#### 립싱크 품질 튜닝 (선택 사항)
워크플로우 JSON에 내장된 값을 덮어씁니다. 입력하지 않으면 기본값이 그대로 사용됩니다.

| 매개변수 | 타입 | 필수 | 기본값 | 설명 |
| --- | --- | --- | --- | --- |
| `audio_scale` | `float` | 아니오 | `1.0` (이미지) / `1.5` (비디오) | 오디오→입모양 조건화 강도. 립싱크에 가장 큰 영향을 주는 값으로, 더 정확한 싱크를 위해 `1.5`~`2.0`까지 올릴 수 있습니다 (너무 높으면 과장될 수 있음). |
| `steps` | `integer` | 아니오 | `6` (이미지) / `4` (비디오) | 디퓨전 샘플링 스텝 수. 많을수록 품질/싱크 향상되지만 느려집니다. |
| `cfg` | `float` | 아니오 | `1.0` | 텍스트 프롬프트에 대한 CFG 가이던스 스케일. |
| `audio_cfg_scale` | `float` | 아니오 | `1.0` | 오디오 조건화에 대한 CFG 가이던스 스케일. |
| `seed` | `integer` | 아니오 | `2` | 재현성을 위한 시드. 값을 바꾸면 다른 결과를 얻습니다. |
| `frame_window_size` | `integer` | 아니오 | `81` | 오디오-영상 정렬에 사용되는 컨텍스트 윈도우(프레임). |
| `motion_frame` | `integer` | 아니오 | `9` | 입모양/모션의 시간적 평활화에 사용되는 프레임 수. |
